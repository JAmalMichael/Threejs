...
export default class Player {
  ...
  update(delta: number) {
    ...
    // // James model also lerps towards the capsules position, but independently of the followCam
    this.animationController?.model?.position.lerp(this.vector, delta * 20)

    // // Also turn James to face the direction of travel.
    // // First, construct a rotation matrix based on the direction from the followTarget to James
    ...
    this.targetQuaternion.setFromRotationMatrix(this.rotationMatrix) // creating a quaternion to rotate James, since eulers can suffer from gimbal lock

    // Next, get the distance from the James model to the followTarget
    const distance = this.animationController?.model?.position.distanceTo(this.followTarget.position)

    // If distance is higher than some espilon, and James quaternion isn't the same as the targetQuaternion, then rotate towards the targetQuaternion.
    ...
    // update which animationAction James should be playing
